...
{
    public function __invoke(Request $request, DashboardSummaryService $dashboard, DashboardLivePayloadFactory $livePayload): View
    {
        return view('dashboard.operations-overview.index', [
            'adminUser' => $request->user(),
            'summary' => $dashboard->summary($request->user()),
            'livePayload' => $livePayload->make($request->user()),
        ]);
    }
}
