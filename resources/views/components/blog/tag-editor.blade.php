@props(['tag' => null, 'locales', 'statuses', 'action', 'method' => 'POST'])

@php
    $isEditing = filled($tag);
    $id = $isEditing ? 'tag-'.$tag->id : 'tag-new';
@endphp
...
<form method="POST" action="{{ $action }}" class="space-y-4" x-data="{ submitting: false }" x-on:submit="if (submitting) { $event.preventDefault(); return; } submitting = true" x-bind:aria-busy="submitting.toString()" x-bind:class="{ 'pointer-events-none opacity-70': submitting }">
    ...
    <div>
        ...
        <select id="{{ $id }}-locale" name="locale_id" class="mt-2 block min-h-11 w-full rounded-lg border border-stone-300 bg-white px-3 text-sm font-bold text-stone-950 shadow-sm focus:border-teal-700 focus:outline-none focus:ring-4 focus:ring-teal-600/20" @error('locale_id') aria-invalid="true" aria-describedby="{{ $id }}-locale-error" @enderror>
            <option value="">Choose language</option>
            @foreach ($locales as $locale)
                <option value="{{ $locale->id }}" @selected((string) old('locale_id', $tag?->locale_id) === (string) $locale->id)>{{ $locale->language_name }}</option>
            @endforeach
        </select>
    ...
    <div>
        ...
        <textarea id="{{ $id }}-description" name="description" rows="4" class="mt-2 block w-full rounded-lg border border-stone-300 bg-white px-3 py-3 text-sm text-stone-950 shadow-sm focus:border-teal-700 focus:outline-none focus:ring-4 focus:ring-teal-600/20" @error('description') aria-invalid="true" aria-describedby="{{ $id }}-description-error" @enderror>{{ old('description', $tag?->description) }}</textarea>
    ...
    <div>
        ...
        <select id="{{ $id }}-status" name="status" class="mt-2 block min-h-11 w-full rounded-lg border border-stone-300 bg-white px-3 text-sm font-bold text-stone-950 shadow-sm focus:border-teal-700 focus:outline-none focus:ring-4 focus:ring-teal-600/20">
            @foreach ($statuses as $value => $label)
                <option value="{{ $value }}" @selected(old('status', $tag?->status ?? 'active') === $value)>{{ $label }}</option>
            @endforeach
        </select>
    </div>
    ...
    <button type="submit" x-bind:disabled="submitting" class="inline-flex min-h-11 w-full items-center justify-center rounded-lg bg-stone-950 px-4 text-sm font-extrabold text-white shadow-sm transition hover:bg-stone-800 focus:outline-none focus:ring-4 focus:ring-teal-600/25 disabled:cursor-not-allowed disabled:opacity-70">
        <span x-text="submitting ? 'Saving...' : '{{ $isEditing ? 'Update tag' : 'Create tag' }}'"></span>
    </button>
</form>
