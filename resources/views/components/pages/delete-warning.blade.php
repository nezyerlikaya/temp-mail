...
<x-admin.card title="Permanent delete readiness" description="Permanent deletion is intentionally separated from trash.">
    <div class="space-y-3 text-sm text-stone-600">
        <p>Only owner/admin-level access can permanently delete a trashed page. This detaches Page Studio media usage and writes an audit event.</p>

        @if ($page->status === 'trashed')
            <form method="POST" action="{{ route('admin.page-studio.destroy', $page) }}" x-data="{ confirmed: false, submitting: false }" x-on:submit="if (submitting || ! confirmed) { $event.preventDefault(); return; } submitting = true">
                @csrf
                @method('DELETE')
                <label class="flex items-start gap-3 rounded-lg border border-red-200 bg-red-50 p-3 text-red-900">
                    ...
                    <span class="text-sm font-bold">I understand this permanently deletes "{{ $page->title }}".</span>
                </label>
        ...
        @else
            <p class="rounded-lg border border-stone-200 bg-stone-50 p-3 font-bold text-stone-700">Move this page to trash before permanent delete readiness is available.</p>
        @endif
    </div>
</x-admin.card>
