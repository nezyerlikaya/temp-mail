...
<div>
    @if ($mediaLibraryReady)
        ...
        />
    @else
        <label for="page-featured-media-id" class="text-sm font-extrabold text-stone-950">Featured media</label>
        <input
            ...
            @error('featured_media_id') aria-invalid="true" aria-describedby="page-featured-media-id-error" @else aria-describedby="page-featured-media-id-help" @enderror
        >
        @error('featured_media_id')
            <p id="page-featured-media-id-error" class="mt-2 text-sm font-bold text-red-700" role="alert">{{ $message }}</p>
        @else
            <p id="page-featured-media-id-help" class="mt-2 text-xs font-bold text-stone-500">Media Library is not available yet, so this accepts an existing media asset ID only.</p>
        @enderror
    @endif
</div>
